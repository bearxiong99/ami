  ...
  methods: {
    ...
    send(val) {
      ...
      readHisTask(val).then(res => {
        ...
        if (res.data.success) {
          ...
          // sessionStorage.setItem("maxData", JSON.stringify(this.taskItem));
          this.$message({
    ...
    checkResult () {
      if(this.taskItem.length===0){
        ...
        let arr = []
        let a=0;
        // JSON.parse(sessionStorage.getItem("maxData"))
    ...
    exportFun(){
      ...
      for (let i = 0; i <= 64; i++) {
        columns['data' + i]=this.$t('maxdata.data' + i)
      }
      this.exportData = {
    ...
    renderHeader(h, { column, $index }){
      ...
      for(let i=0;i<this.obisArr.length;i++){
        if (label === this.$t('maxdataT.data'+i)){
          return h('p',{
            on: {
              mouseenter: (e) => {
                if (ua.match(/firefox/) != null) {
                  e.target.title=`${this.$t('maxdata.data'+i)}\n${this.obisArr[i]}`;
                }else if (ua.match(/chrome/) != null){
                  e.path[0].title = `${this.$t('maxdata.data'+i)}\n${this.obisArr[i]}`
                }
    ...
    getcolumnList () {
      ...
      for (let i = 0; i < 65; i++) {
        this.columnList.push({
          datavalue: 'data' + i,
          descript: this.$t('maxdataT.data' + i),
          obis: this.obisArr[i]
        });
      }
    },
