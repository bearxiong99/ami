          ...
          >
              <!-- <el-table-column :prop="'data' + index" :label="item.obis" > -->
                <template slot-scope="scope">
                  <div v-if="index === 13 || index === 22 || index === 31 || index === 40">{{changeVal(scope.row['data'+index])}}</div>
...
export default {
  ...
  methods: {
    ...
    read4gFromMeter(){
      ...
      this.selectTableData.forEach(item => {
        taskItems.push({
          ...
          startTime:this.formatTimeT(this.loadForm.timeStart)+' 00:00:00',
          endTime:this.formatTimeT(this.loadForm.timeEnd)+' 23:59:59'
        });
      });
      this.send(taskItems);
    },
    ...
    send(val) {
      ...
      readHisTask(val).then(res => {
        ...
        if (res.data.success) {
          ...
          if(this.isDirect===0){
            // sessionStorage.setItem("dayDataA", JSON.stringify(this.taskItem));
            this.dayDataA=this.taskItem;
          }else if(this.isDirect===1){
            // sessionStorage.setItem("dayDataB", JSON.stringify(this.taskItem));
            this.dayDataB=this.taskItem;
          }
          this.$message({
    ...
    checkResult () {
      if(this.dayDataA.length!=0||this.dayDataB.length!=0){
        ...
        let arr = []
        if(this.isDirect===0){
          this.dayDataA.forEach(item=>{
            arr.push(item.taskId)
          })
        }else if(this.isDirect===1){
          this.dayDataB.forEach(item=>{
            arr.push(item.taskId)
          })
        }
        this.paramValue={
    ...
    exportFun(){
      ...
      for (let i = 0; i <= 40; i++) {
        columns['data' + i]=this.$t('daydata.data' + i)
      }
      this.exportData = {
        url: "/api/ami-communicate/taskCore/checkTaskResultList",
        params: this.paramValue,
        columns: columns,
        fileName: "maxinumdemandquistion", // avgvolcur- + 对象类型- +对象名称命名
        sheetName: "maxinumdemandquistion"
      };
    },
    changeVal(val){
      if (isNaN(val)) {
    ...
    renderHeader(h, { column, $index }){
      ...
      for(let i=0;i<this.obisArr.length;i++){
        if (label === this.$t('daydataT.data'+i)){
          return h('p',{
            on: {
              mouseenter: (e) => {
                if (ua.match(/firefox/) != null) {
                  e.target.title=`${this.$t('daydata.data'+i)}\n${this.obisArr[i]}`;
                }else if (ua.match(/chrome/) != null){
                  e.path[0].title = `${this.$t('daydata.data'+i)}\n${this.obisArr[i]}`
                }
    ...
    getcolumnList () {
      ...
      for (let i = 0; i <= 40; i++) {
        this.columnList.push({
          datavalue: 'data' + i,
          descript: this.$t('daydataT.data' + i),
          obis: this.obisArr[i]
        });
      }
    },
