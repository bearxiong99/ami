<template>
  <div>
    <div class="select">
      <el-checkbox v-model="meterReadingParams" class="all">{{$t('global.mrp')}}</el-checkbox>
      <div class="group">
        <div class="params">
          <span>Cause for last auto reset </span>
          <el-checkbox-group v-model="params1">
            <el-checkbox :label="ele.val" v-for="ele in arr1" :key="ele.val">{{ele.label}}</el-checkbox>
          </el-checkbox-group>
        </div>
        <div class="params">
          <span>Cause for last demand reset </span>
          <el-checkbox-group v-model="params2">
            <el-checkbox :label="ele.val" v-for="ele in arr2" :key="ele.val">{{ele.label}}</el-checkbox>
          </el-checkbox-group>
        </div>
        <div class="params">
          <span>Duration of prohibiting "manual reset" after previous demand reset(s)</span>
          <el-input v-model="params3" style="width:200px"></el-input>
        </div>
      </div>
    </div>
...
export default {
  ...
  data () {
    return {
      meterReadingParams: false,
      basicdata:[],
      selectTableData:[],
      resultTableData:[],
      tableData:[],
      params1: [1,2,3,4,5,6,7,8],
      params2: [1,2,3,4,5,6,7,8],
      params3: '',
      arr1: [
        {val: 1, label: 'Select all'},
        {val: 2, label: 'Reserved'},
        {val: 3, label: 'Pass a date(or a month)'},
        {val: 4, label: 'By internal clock schedule'},
        {val: 5, label: 'On command via the communication interfaces'},
        {val: 6, label: 'By date/time change'},
        {val: 7, label: 'Program change'},
        {val: 8, label: 'Reserved'},
      ],
      arr2: [
        {val: 1, label: 'Select all'},
        {val: 2, label: 'Reserved'},
        {val: 3, label: 'Pass a month'},
        {val: 4, label: 'By internal clock schedule'},
        {val: 5, label: 'On command via the communication interfaces'},
        {val: 6, label: 'By date/time change'},
        {val: 7, label: 'Program change'},
        {val: 8, label: 'By manual push-button'},
      ],
  ...
  methods:{
    ...
    checkAll(val) {
      this.checked = val ? [1, 2, 3] : [];
      this.isIndeterminate = false;
    },
    check(val) {
      let length = val.length;
      this.checkedAll = length === 3;
